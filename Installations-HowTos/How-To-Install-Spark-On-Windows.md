...
* Choose a Spark release: 3.5.0 (Sep 13 2023)<br/>
* Choose a package type: Pre-built for Hadoop 3.3<br/>
*Please note that Spark and Hadoop are regularly updated & therefore the actual versions will differ*

Create a folder 'spark' in 'C:\ProgramData' and move 'spark-3.5.0-bin-hadoop3.tgz' from your downloads folder to this directory.
Now uncompress the file using 7zip, so that you have the following directory structure:
```
 Directory of C:\ProgramData\spark\spark-3.5.0-bin-hadoop3
...
```
SPARK_HOME = C:\ProgramData\spark\spark-3.5.0-bin-hadoop3
HADOOP_HOME = C:\ProgramData\spark\spark-3.5.0-bin-hadoop3
```
Append the following path to your system environment variable Path
```
C:\ProgramData\spark\spark-3.5.0-bin-hadoop3\bin
```

Download winutils.exe corresponding to your hadoop versionFrom the hadoop binaries repo:<br/>
https://github.com/cdarlint/winutils<br/>
In our case we need to get the winutils.exe from here:<br/>
https://github.com/cdarlint/winutils/tree/master/hadoop-3.3.5/bin
Save winutils.exe in the ```C:\ProgramData\spark\spark-3.3.5-bin-hadoop3\bin``` directory:

Now, create the an empty folder ```hive``` in the ```C:\tmp``` directory

Finally, we need to install pyspark bindings for Anaconda. To do this, fire up **Anaconda Propmpt** in administrator mode and type in the following command:
```
